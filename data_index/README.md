...
`manifest.json`,由 `EXPERIMENTS/scripts/regen_manifest.py` 生成 —— **勿手改**。

## 六个 index 一览

| index | 对数 | 楼 | 图像来源 | 一行的格式 | D | 用途 |
|---|---|---|---|---|---|---|
| `train_balanced_32768` | 32,768 | 631 | PairUAV `train_tour/` | `<楼>/<帧A>_<帧B>.json` | 全合法 | **训练**(按 D 均衡,占全量 1.60%) |
...
> 三个 `val10_*` 与 `val_quick_2048` **同 70 栋楼**,且 |D| 分布已匹配(均值 128.1 / 128.5 / 128.4 vs 133.2),故可横向比较。
...
```bash
# index 的 sha256 / 楼数 / D 分布
python EXPERIMENTS/scripts/regen_manifest.py    # 重建 manifest.json,与 git 里的 diff 应为空
